...
void CFEPAnalyzer<FO>::createIBIMarker(OutlinedFunction *OutlinedFunction,
  ...
  // Traverse all the indirect jump of the function (exit point). For each one
  // of them, create a dedicated basic block (their successor is `anypc` BB) and
  // load the values of the stack pointer, program counter and ABI registers at
  // function end.
  for (auto *Term : SV) {
    ...
    // function. Are we returning properly?
    auto *JumpsToReturnAddress = IRB.CreateSub(PCE, PCI);

    // Compute the difference between the stack pointer values to evaluate
    // the stack height, and the difference between the CSR values as well.
    // Functions leaving the stack pointer higher than it was at function
    // entry (i.e., in an irregular state) will be marked as fake functions.
    ...
    // Compute the difference between the initial and final values of the ABI
    // registers, in order to find which are callee-saved.
    ...
    // Materialize the `indirect_branch_info` call
...
void CFEPAnalyzer<FO>::runOptimizationPipeline(llvm::Function *F) {
  ...
  // TODO: break it down in the future, and check if some passes can be dropped
  {
    ...
    // geps. Since the IR may change remarkably, another round of passes is
    // necessary to take more optimization opportunities.
...
llvm::Function *CFEPAnalyzer<FO>::createFakeFunction(llvm::BasicBlock *Entry) {
  ...
  // Fake functions have one and only one broken return
...
FunctionSummary CFEPAnalyzer<FO>::analyze(BasicBlock *Entry) {
  ...
  // Detect boundaries of a function
  ...
  // i.e., it jumps to the return address. To achieve this, we craft the IR by
  // loading the program counter, the stack pointer, as well as the ABI
  // registers CSVs respectively at function prologue / epilogue. When the
  // subtraction between their entry and end values is found to be zero (after
  // running an optimization pipeline), we know it properly returns, leaves the
  // stack unmodified, etc. The marker looks as follows and is installed upon
  // every indirect jump:
  //
  //                           jumps to RA,    SP,    rax,   rbx,   rbp
  // call i64 @indirect_branch_info(i128 0, i64 8, i64 %8, i64 0, i64 0)
  //
  // In this case we can infer that the function jumps to its return address
  // (it is not a longjmp / tail call), `rax` register has been clobbered by
  // the callee, whereas `rbx` and `rbp` are callee-saved registers.
  ...
  // Execute an optimization pipeline over the disposable function
  ...
  // Is the outlined function a FakeFunction? If so, return a copy of the
  // unoptimized outlined function. When analyzing the caller, this function
  // will be inlined in its call-site.
...
CFEPAnalyzer<FO>::milkInfo(OutlinedFunction *OutlinedFunction,
  ...
  // Function elected fake if there is one and only one broken return
...
void CFEPAnalyzer<FunctionOracle>::integrateFunctionCallee(llvm::BasicBlock *BB,
  ...
  case FunctionTypeValue::NoReturn: {
    ...
    // Different insertion point depending on the callee type
    IRBuilder<> Builder(M.getContext());
...
OutlinedFunction CFEPAnalyzer<FO>::outlineFunction(llvm::BasicBlock *Entry) {
  ...
  // Cleanup: fix `unexpectedpc` of the fake callee

...
template<typename T, bool HasAddend>
void ELFImporter<T, HasAddend>::recordSymbol(const Elf_Sym_Impl<T> &Symbol) {
  bool IsIfunc = Symbol.getType() == ELF::STT_GNU_IFUNC;
  auto Inserted = SymbolsByAddress
                    .try_emplace(Symbol.st_value,
                                 LDDTree::Symbol{ .Address = Symbol.st_value,
                                                  .IsIfunc = IsIfunc })
                    .second;
  if (not Inserted) {
    revng_log(ELFImporterLog,
              "Duplicate symbol entry at 0x"
                << llvm::utohexstr(Symbol.st_value, true) << " ignored");
  }
}

  ...
  outputs =
    ...
    in
    {
      packages.${system} = {
        ...
        "test/revng-prss" = stdenv.mkDerivation {
          ...
          installPhase = ''
            ...
            # --install-path.
            mkdir merged-root
            lndir -silent ${self.packages.${system}.revng-qa} merged-root
            lndir -silent ${self.packages.${system}."test/revng-qa"} merged-root
            lndir -silent ${self.packages.${system}.revng} merged-root

            python3 \
              ${self.packages.${system}.revng-qa}/libexec/revng/test-configure \
              "${self.packages.${system}.revng-qa}/share/revng/test/configuration/revng-qa/"*.yml \
              "${self.packages.${system}.revng}/share/revng/test/configuration/revng-prss/"*.yml \
              --install-path "$PWD/merged-root" \
              --destination . \

...
VH::dropInterArgumentPadding(llvm::ArrayRef<std::byte> Bytes) const {
  ...
  size_t PreviousArgumentEndsAt = 0;
...
bool VH::tryToVerifyStack(llvm::ArrayRef<std::byte> &Bytes,
  ...
  if (Bytes.take_front(ExpectedBytes.size()).equals(ExpectedBytes)) {
    size_t BytesToDrop = ABI.paddedSizeOnStack(ExpectedBytes.size());
...
uint64_t VH::valueFromBytes(llvm::ArrayRef<std::byte> Input) const {
  size_t PointerSize = model::Architecture::getPointerSize(Architecture);
  ...
  for (size_t I = 0; I < PointerSize; ++I) {
    ...
    size_t Index = IsLittleEndian ? (PointerSize - I - 1) : I;
    if (Index < Input.size())
